...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(noise_dim+class_num, 128)
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(28*28+class_num, 512)
...
class GAN():
    ...
    def study(self, epoch):
        ...
        for repeat, (real_data, real_label) in enumerate(self.dataloader, 1):
            # train data
            if repeat * mini_batch_num < data_num*(1-valid_rate):
                ...
                real_data, _ = self.reshape_input(real_data, label=real_label)
                ...
                input_noise, fake_label = self.reshape_input(input_noise)
                fake_data = self.generator(input_noise)
                fake_data, _ = self.reshape_input(fake_data, fake_label)
                ...
                """
                # 1000ごとに表示
                if mini_batch_num*repeat%1000 == 0:
            ...
            # valid data
                ...
                input_noise, input_label = self.reshape_input(input_noise)
                fake_data = self.generator(input_noise)
                fake_data, _ = self.reshape_input(fake_data, input_label)
        ...
        # renew weight if valid acc improve
    ...
    def evaluate(self):
        ...
        for repeat, (real_data, real_label) in enumerate(self.dataloader, 1):
            ...
            real_data, _ = self.reshape_input(real_data, real_label)
    ...
    def eval_pic(self, epoch):
        ...
        for col in range(1, pic_size+1):
            ...
            for row in range(1, pic_size+1):
                ...
                input_noise, _ = self.reshape_input(input_noise, [col-1])
    ...
    # convert label to one hot and concat data
    # if label is None, generate random label
    def reshape_input(self, input, label=None):
        input = input.view(input.shape[0], -1)
        onehot_label = torch.zeros(input.shape[0], class_num)
        if label is None:
            label = torch.randint(0, class_num, (input.shape[0],))
        for i, label_num in enumerate(label):
            #print(i, label_num)
            onehot_label[i, int(label_num)] = 1
        input = torch.cat((input, onehot_label), dim=1)
        return input, label
    ...
    def output(self):
        plt.figure()
        plt.plot(range(epochs), self.g_loss_transition)
        plt.xlabel("epochs")
        plt.ylabel("g loss")
        plt.savefig("./result/g_loss_transition.png")

        plt.figure()
        plt.plot(range(epochs), self.d_loss_transition)
        plt.xlabel("epochs")
        plt.ylabel("d loss")
        plt.savefig("./result/d_loss_transition.png")
